...
class Experiment:
    def __init__(self, args=None, dataset=None, train_mask=None, validation_mask=None, test_mask=None):
        ...
        if self.test_mask is None:
            ...
            train_size = int(self.args.train_fraction * self.num_nodes)
            validation_size = int(self.args.validation_fraction * self.num_nodes)
            test_size = self.num_nodes - train_size - validation_size

            self.train_mask, self.validation_mask, self.test_mask = random_split(node_indices,[train_size, validation_size, test_size])
...
#                        if epoch > 10:
#                            SMVList = self.check_SMV(batch=batch)
...
#                            SMVList = self.check_SMV(batch=batch)
                ...
                if epochs_no_improve > self.args.patience:
                    ...
                    return train_acc, validation_acc, test_acc, SMVList, best_test_acc
    ...
    def check_SMV(self, batch):
        self.model.eval()
        with torch.no_grad():
            SMVList = self.model(batch, measure_SMV = True)

            return np.array(SMVList)